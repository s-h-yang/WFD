...
Z = zeros(n,1)
X = readdlm("data/"*dataset*"/attributes_pca_gc.txt", ' ', Float64, '\n')
...
for c in 1:num_classes
    ...
    for i = 1:min(100,length(K))
        ...
        for alpha = 1.5:0.25:5
            ...
            x = WFD(G, source, sink, weights=edge_weight_z, max_iters=50, epsilon=1.0e-4)
            C = findall(!iszero,x)
            ...
            x = WFD(G, source, sink, weights=edge_weight_x, max_iters=50, epsilon=1.0e-4)
            C = findall(!iszero,x)
            pr = length(intersect(Set(C),Set(K)))/length(C)
            re = length(intersect(Set(C),Set(K)))/k
            f1 = 2*pr*re/(pr+re)
...
open("F1-coauthor-phy-base.txt", "w") do f
    for i in 1:num_classes
        @printf(f, "%.4f\n", mean(BASE_results[i]))
    end
end

open("F1-coauthor-phy-wfd.txt", "w") do f
    for i in 1:num_classes
        @printf(f, "%.4f\n", mean(WFD_results[i]))
    end
end
